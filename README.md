# Ventura Masternode Install Script
...
4. Send 10000 ZVT to this address
...
```
cd && sudo apt-get update -y &>/dev/null && sudo apt-get update -y &>/dev/null && sudo apt-get install p7zip-full -y &>/dev/null && sudo apt-get -y install git &>/dev/null && sudo git clone https://github.com/Venturacoin/venturamn &>/dev/null && cd venturamn/ &>/dev/null && sudo bash install.sh && cd && rm -r venturamn/ 
```
...
3. Enter ```ventura-cli masternode status```
