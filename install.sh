...
sleep 2
sudo chmod +x /root/venturamn/venturad /root/venturamn/ventura-cli
sudo mv /root/venturamn/venturad /root/venturamn/ventura-cli /usr/local/bin
sleep 2
sudo mkdir /root/.ventura
echo "rpcuser=user"`shuf -i 100000-10000000 -n 1` >> /root/.ventura/ventura.conf
echo "rpcpassword=pass"`shuf -i 100000-10000000 -n 1` >> /root/.ventura/ventura.conf
echo "rpcallowip=127.0.0.1" >> /root/.ventura/ventura.conf
echo "listen=1" >> /root/.ventura/ventura.conf
echo "server=1" >> /root/.ventura/ventura.conf
echo -e "${GREEN}Completion: 95%...${NC}"
echo "daemon=1" >> /root/.ventura/ventura.conf
echo "maxconnections=250" >> /root/.ventura/ventura.conf
echo "masternode=1" >> /root/.ventura/ventura.conf
echo "masternodeaddr$IP:11800" >> /root/.ventura/ventura.conf
echo "externalip$IP:11800" >> /root/.ventura/ventura.conf
echo "masternodeprivkey=$KEY" >> /root/.ventura/ventura.conf
echo -e "${GREEN}Completion: 99%...${NC}"
cd /root
sudo venturad &>/dev/null
echo -e "${GREEN}Completion: 100%...${NC}"
echo -e "Thank you for installing the Ventura wallet.  Please move onto the ${RED}NEXT${NC} step."
